//
//  AuthWorkInfoViewController.swift
//  NewNewStarLoan
//
...
class AuthWorkInfoViewController: UIViewController {
  // MARK: properties
  private let viewModel: AuthWorkInfoViewModel
  var workInfo: GetWorkInfoResponse?
  ...
  // MARK: - Initialization
  init(viewModel: AuthWorkInfoViewModel) {
...
extension AuthWorkInfoViewController {
  private func setupBelowUI() {
    ...
    // 设置标题
    navigationBar.setTitle("Work Information")
...
extension AuthWorkInfoViewController: UITableViewDataSource, UITableViewDelegate {
...
//        cell.updateSelectedOption(matching as! BadDisplayable)
      }
...
extension AuthWorkInfoViewController {
  @objc private func nextStepButtonTapped() {
    ...
    Task {
      do {
        let out = try await viewModel.saveWorkInfo()
        ...
        if cousin == "nbaallstari" {
          Task {
            do {
              let response = try await CertificateService.shared.getContactInfo(feud: viewModel.productId)  // 使用 productId
              await MainActor.run {
